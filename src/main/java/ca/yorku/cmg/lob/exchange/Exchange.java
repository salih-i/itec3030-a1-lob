...
public class Exchange {
	...
	public boolean validateOrder(IOrder o) {
		...
		if (this.accounts.getTraderAccount(o.getTrader()) == null) {
		...
		int pos =this.accounts.getTraderAccount(o.getTrader()).getPosition(o.getSecurity().getTicker());;
		//Get the balance the trader has with the exchange
		long bal =this.accounts.getTraderAccount(o.getTrader()).getBalance();;
	...
	public void submitOrder(IOrder o, long time) {
		...
		if (o instanceof Bid) {// Order is a bid
			...
			oOutcome = this.book.getAsks().processOrder(o, time);
		...
		for (ITrade t:oOutcome.getResultingTrades()) {
			...
			t.setBuyerFee(this.accounts.getTraderAccount(t.getBuyer()).getFee(t));
			//Apply the above fee to the account balance of the buyer 			
			this.accounts.getTraderAccount(t.getBuyer()).applyFee(t);
			//Apply the trade payment to the account balance of the buyer (they spent money)
			this.accounts.getTraderAccount(t.getBuyer()).withdrawMoney(t.getValue());
			//Add the bought stocks to the position of the buyer
			this.accounts.getTraderAccount(t.getBuyer()).addToPosition(t.getSecurity().getTicker(), t.getQuantity());
			
			//Update balances for Seller
			
			//Get the fee that the seller is supposed to pay
			t.setSellerFee(this.accounts.getTraderAccount(t.getSeller()).getFee(t));
			//Apply the above fee to the account balance of the seller
			this.accounts.getTraderAccount(t.getSeller()).applyFee(t);;
			//Apply the trade payment to the account balance of the seller (they earned money)
			this.accounts.getTraderAccount(t.getSeller()).addMoney(t.getValue());
			//Deduct the sold stocks from the position of the seller
			this.accounts.getTraderAccount(t.getSeller()).deductFromPosition(t.getSecurity().getTicker(), t.getQuantity());
